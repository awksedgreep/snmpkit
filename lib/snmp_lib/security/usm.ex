defmodule SnmpLib.Security.USM do
  @moduledoc """
  ...
  ### Engine Discovery
      ...
      {:ok, engine_id} = SnmpLib.Security.USM.discover_engine("192.168.1.1")

      # Time synchronization
      {:ok, {boots, time}} = SnmpLib.Security.USM.synchronize_time("192.168.1.1", engine_id)

  ### Message Processing

      # Process outgoing secure message
      {:ok, secure_message} = SnmpLib.Security.USM.process_outgoing_message(
        user, message, security_level
      )

      # Process incoming secure message
      {:ok, {plain_message, user}} = SnmpLib.Security.USM.process_incoming_message(
