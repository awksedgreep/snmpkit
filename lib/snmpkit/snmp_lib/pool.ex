defmodule SnmpKit.SnmpLib.Pool do
  @moduledoc """
  ...
  def init({pool_name, opts}) do
    ...
    case initialize_connections(state) do
      {:ok, connections} ->
        ...
        Logger.info(
          "Started SNMP connection pool #{pool_name} with #{length(connections)} connections"
        )
  ...
  def handle_cast({:record_success, _connection, response_time}, state) do
    ...
    # Keep last 100
    new_times = [response_time | Enum.take(state.response_times, 99)]
  ...
  defp create_connection(worker_opts) do
    case SnmpKit.SnmpLib.Transport.create_client_socket(worker_opts) do
      {:ok, socket} ->
        connection = %{
          socket: socket,
          pid: self(),
  ...
  defp close_connection(connection) do
    SnmpKit.SnmpLib.Transport.close_socket(connection.socket)
  end
  ...
  defp perform_health_checks(state) do
    ...
    updated_connections =
      Enum.map(all_connections, fn connection ->
    ...
    %{state | connections: main_conns, overflow_connections: overflow_conns}
  end
  ...
  defp check_connection_health(connection) do
    ...
    cond do
      ...
      # 10 minutes idle
      current_time - connection.last_used > 600_000 -> :degraded
      true -> :healthy
    end
  end
  ...
  defp calculate_pool_stats(state) do
    ...
    health_status =
      Enum.group_by(all_connections, & &1.health_status)
      |> Enum.map(fn {status, conns} -> {status, length(conns)} end)
      |> Enum.into(%{})

    avg_response_time =
      case state.response_times do
        [] -> 0.0
