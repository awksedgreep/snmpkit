defmodule SnmpKit.SnmpLib.Dashboard do
  @moduledoc """
  ...
  def init(opts) do
    ...
    Logger.info("SnmpKit.SnmpLib.Dashboard started on port #{state.port}")
    {:ok, state}
  end
    ...
    %{
      ...
      avg_response_time:
        if(length(response_times) > 0,
          do: Enum.sum(response_times) / length(response_times),
          else: 0.0
        ),
  ...
  defp get_current_pool_utilization do
    # This would get actual pool utilization from SnmpKit.SnmpLib.Pool
