defmodule SnmpKit.SnmpLib.Transport do
  @moduledoc """
      ...
      {:ok, resolved_address} ->
        client_options =
          Keyword.merge(
            ...
            [
              {:active, false},
              # Larger receive buffer for responses
              {:recbuf, 65536},
  ...
  def create_server_socket(port, bind_address \\ "0.0.0.0") do
    server_options = [
      {:active, false},
      # Smaller receive buffer for requests
      {:recbuf, 8192},
      # Larger send buffer for responses
      {:sndbuf, 65536}
    ]
  ...
  def receive_packet(socket, timeout \\ 5000) when is_integer(timeout) and timeout >= 0 do
    case :gen_udp.recv(socket, 0, timeout) do
      {:ok, {from_address, from_port, data}} when is_binary(data) ->
        Logger.debug(
          "Received #{byte_size(data)} bytes from #{format_endpoint(from_address, from_port)}"
        )
    ...
    case create_client_socket() do
      {:ok, socket} ->
        try do
          ...
          test_packet = <<0x30, 0x02, 0x01, 0x00>>
