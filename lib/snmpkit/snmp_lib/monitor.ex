defmodule SnmpKit.SnmpLib.Monitor do
  @moduledoc """
  ...
  @default_retention_period 3_600_000
  # 1 minute buckets
  ...
  def init(opts) do
    ...
    final_state = %{state | cleanup_timer: cleanup_timer, health_check_timer: health_timer}
  ...
  def handle_cast({:set_alert_threshold, alert_config}, state) do
    ...
    Logger.info(
      "Set alert threshold for " <>
        alert_config.device_id <>
        " " <> to_string(alert_config.metric) <> ": " <> to_string(alert_config.threshold)
    )
  ...
  def handle_cast({:remove_alert_threshold, device_id, metric}, state) do
    new_thresholds =
      Enum.reject(state.alert_thresholds, fn threshold ->
  ...
  defp update_response_times(stats, metric) do
    # Keep last 100
    new_times = [metric.duration | Enum.take(stats.response_times, 99)]
  ...
  defp check_alert_conditions(state, metric) do
    ...
    new_alerts =
      Enum.reduce(state.alert_thresholds, state.active_alerts, fn threshold, alerts ->
  ...
  defp cleanup_old_data(state) do
    ...
    %{state | operations: new_operations, device_stats: new_device_stats}
  end
  ...
  defp calculate_operation_metrics(state, operation, timeframe) do
    ...
    filtered_ops =
      filter_operations_by_timeframe(state.operations, timeframe)
  ...
  defp export_csv(operations) do
    ...
    rows =
      Enum.map(operations, fn op ->
        to_string(op.timestamp) <>
          "," <>
          op.device <>
          "," <>
          to_string(op.operation) <>
          "," <>
          to_string(op.duration) <>
          "," <> to_string(op.result) <> "," <> to_string(op[:error_type] || "")
      end)
      |> Enum.join("\n")
  ...
  defp export_prometheus(state) do
    ...
    "# HELP snmp_operations_total Total number of SNMP operations\n" <>
      ...
      to_string(system_stats.total_operations) <>
      "\n\n" <>
      "# HELP snmp_operations_per_second Current operations per second\n" <>
      "# TYPE snmp_operations_per_second gauge\n" <>
      "snmp_operations_per_second " <>
      to_string(system_stats.operations_per_second) <>
      "\n\n" <>
