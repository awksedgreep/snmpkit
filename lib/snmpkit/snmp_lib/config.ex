defmodule SnmpKit.SnmpLib.Config do
  @moduledoc """
  ...
  def init(opts) do
    ...
    if Keyword.get(opts, :validate_on_start, true) do
      case validate_config(initial_config) do
        {:ok, _} ->
          Logger.info("SnmpKit.SnmpLib.Config started with valid configuration")

        {:error, errors} ->
          Logger.warning(
            "SnmpKit.SnmpLib.Config started with validation errors: #{inspect(errors)}"
          )
      end
    end
