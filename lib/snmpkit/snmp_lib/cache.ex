defmodule SnmpKit.SnmpLib.Cache do
  @moduledoc """
  ...
  def init(opts) do
    ...
    Logger.info("SnmpKit.SnmpLib.Cache started with max_size=#{state.max_size}")
    {:ok, state}
  end
  ...
  defp calculate_adaptive_ttl(key, base_ttl, volatility) do
    ...
    access_multiplier =
      case access_pattern do
        ...
        :frequent -> 0.8
        :normal -> 1.0
        # Longer TTL for rarely accessed data
        :rare -> 1.5
      end
  ...
  defp invalidate_by_pattern(pattern) do
    ...
    Logger.debug(
      "Invalidated #{length(matching_keys)} cache entries matching pattern '#{pattern}'"
    )
  end
  ...
  defp perform_cleanup(state) do
    ...
    # 1 hour
    cleanup_threshold = current_time - 3_600_000
    ...
    Logger.debug(
      "Cache cleanup: removed #{length(expired_keys)} expired entries and #{length(old_access_records)} old access records"
    )
  ...
  defp evict_lru_entries(state) do
    ...
    lru_entries =
      ...
      # Evict 10% of cache
      |> Enum.take(div(state.max_size, 10))
