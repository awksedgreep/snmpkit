defmodule SnmpKit.SnmpLib.ErrorHandler do
  @moduledoc """
  ...
  def init({:circuit_breaker, device_id, opts}) do
    state = %{
      ...
      opts: %{
        ...
        half_open_max_calls:
          Keyword.get(opts, :half_open_max_calls, @default_half_open_max_calls),
    ...
    try do
      case fun.() do
        ...
        {:error, reason} = error ->
          if config.attempt < config.max_attempts and config.retry_condition.(reason) do
            ...
            Logger.debug(
              "Retry attempt #{config.attempt} failed: #{inspect(reason)}, waiting #{delay}ms"
            )
      ...
      exception ->
        if config.attempt < config.max_attempts do
          ...
          Logger.debug(
            "Retry attempt #{config.attempt} raised: #{inspect(exception)}, waiting #{delay}ms"
          )
  ...
  defp record_failure(state, _reason) do
    ...
    cond do
      ...
      state.state == :closed and new_failure_count >= state.opts.failure_threshold ->
        ...
        Logger.warning(
          "Circuit breaker opened for device #{state.device_id} after #{new_failure_count} failures"
        )
