defmodule SnmpKit.SnmpKit.SnmpMgr.Multi do
  ...
  def get_multi(targets_and_oids, opts \\ []) do
    ...
    execute_concurrent_requests(targets_and_oids, timeout, max_concurrent, fn {target, oid,
      ...
      SnmpKit.SnmpMgr.get(target, oid, merged_opts)
    end)
  end
  ...
  def get_bulk_multi(targets_and_oids, opts \\ []) do
    ...
    execute_concurrent_requests(targets_and_oids, timeout, max_concurrent, fn {target, oid,
                                                                               request_opts} ->
      merged_opts = Keyword.merge(opts, request_opts)
      SnmpKit.SnmpMgr.get_bulk(target, oid, merged_opts)
    end)
  end
  ...
  def walk_table_multi(targets_and_tables, opts \\ []) do
    ...
    execute_concurrent_requests(targets_and_tables, timeout, max_concurrent, fn {target,
                                                                                 table_oid,
                                                                                 request_opts} ->
      merged_opts = Keyword.merge(opts, request_opts)
      SnmpKit.SnmpMgr.walk_table(target, table_oid, merged_opts)
    end)
  end
  ...
  defp execute_operation(:get_next, target, oid, opts) do
    SnmpKit.SnmpMgr.get_next(target, oid, opts)
  end

  defp execute_operation(:set, target, {oid, value}, opts) do
    SnmpKit.SnmpMgr.set(target, oid, value, opts)
  end
  ...
  defp execute_operation(:walk_table, target, table_oid, opts) do
    SnmpKit.SnmpMgr.walk_table(target, table_oid, opts)
  end
