defmodule SnmpKit.SnmpMgr.PerformanceBenchmark do
  ...
  def measure_throughput(opts \\ []) do
    ...
    requests_per_second =
      if actual_duration > 0, do: total_requests / (actual_duration / 1000), else: 0
  ...
  def monitor_buffer_usage(targets, max_concurrent, timeout) do
    ...
    utilizations =
      Enum.map(final_buffer_samples, fn sample ->
    ...
    %{
      ...
      avg_utilization:
        if(utilizations != [], do: Enum.sum(utilizations) / length(utilizations), else: 0),
    ...
    for {device, index} <- Enum.with_index(devices, 1) do
      ...
      case wait_for_device_ping(host, device.community, 5000) do
        ...
        {:error, reason} ->
          ...
          # Continue anyway for benchmarking
          :ok
      end
    end
