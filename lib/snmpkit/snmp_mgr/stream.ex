defmodule SnmpKit.SnmpKit.SnmpMgr.Stream do
  @moduledoc """
  ...
  ## Examples
      ...
      "switch.local"
      |> SnmpKit.SnmpKit.SnmpMgr.Stream.walk_stream("ifTable")
      ...
      "router.local"
      |> SnmpKit.SnmpKit.SnmpMgr.Stream.walk_stream("ipRouteTable", chunk_size: 100)
  ...
  ## Examples
      ...
      "switch.local"
      |> SnmpKit.SnmpKit.SnmpMgr.Stream.table_stream("ifTable", columns: [:ifDescr, :ifOperStatus])
      ...
      "device.local"
      |> SnmpKit.SnmpKit.SnmpMgr.Stream.table_stream("ipRouteTable", chunk_size: 200)
  ...
  ## Examples
      ...
      targets
      |> SnmpKit.SnmpKit.SnmpMgr.Stream.monitor_stream(interval: 30_000)
      |> Stream.each(&send_to_metrics_system/1)
      |> Stream.run()

      # Monitor with error handling
      targets
      |> SnmpKit.SnmpKit.SnmpMgr.Stream.monitor_stream(
           interval: 10_000,
  ...
  ## Examples
      ...
      operations
      |> SnmpKit.SnmpKit.SnmpMgr.Stream.concurrent_stream(max_concurrent: 3)
  ...
  def concurrent_stream(device_operations, opts \\ []) do
    ...
    Stream.resource(
      fn ->
        initialize_concurrent_stream(device_operations, max_concurrent, ordered, timeout, opts)
      end,
  ...
  ## Examples
      ...
      "switch.local"
      |> SnmpKit.SnmpKit.SnmpMgr.Stream.filtered_stream("ifTable", filter_fn)
      |> Enum.to_list()
  """
  ...
  defp fetch_next_chunk(state, opts) do
    chunk_size =
      if state.adaptive and state.adaptive_state do
    ...
    case SnmpKit.SnmpKit.SnmpMgr.Core.send_get_bulk_request(state.target, state.current_oid, bulk_opts) do
      {:ok, results} ->
      ...
      {:error, reason} ->
        if state.adaptive and state.adaptive_state.current_size > 1 do
          ...
          new_adaptive_state = %{
            state.adaptive_state
            | current_size: div(state.adaptive_state.current_size, 2)
          }
  ...
  defp fetch_next_table_chunk(%{error: reason}, _opts),
    do: {[{:error, reason}], %{finished: true}}
  ...
  defp fetch_next_monitor_data(state, opts) do
    ...
    if current_time - state.last_poll >= state.interval do
      ...
      new_buffer =
        Enum.reduce(poll_results, state.buffer, fn result, acc ->
          :queue.in(result, acc)
        end)
  ...
  defp fetch_next_concurrent_data(state, opts) do
    ...
    updated_results =
      Enum.reduce(completed, new_state.completed_results, fn result, acc ->
        :queue.in(result, acc)
      end)
    ...
    final_state = %{new_state | active_tasks: still_active, completed_results: remaining_results}

    if Enum.empty?(items) and Enum.empty?(still_active) and
         final_state.operation_index >= length(final_state.operations) do
      {:halt, final_state}
    else
      {items, final_state}
    end
  ...
  defp poll_targets(targets, error_handling, opts) do
    SnmpKit.SnmpKit.SnmpMgr.Multi.get_multi(targets, opts)
    |> Enum.zip(targets)
    |> Enum.map(fn {result, {target, oid}} ->
      case result do
        {:ok, value} ->
          {:ok, %{target: target, oid: oid, value: value, timestamp: System.system_time(:second)}}

        {:error, reason} when error_handling == :include_errors ->
          {:error,
           %{target: target, oid: oid, error: reason, timestamp: System.system_time(:second)}}
  ...
  defp start_concurrent_tasks(state, opts) do
    ...
    operations_to_start =
      Enum.drop(state.operations, state.operation_index) |> Enum.take(available_slots)
  ...
  defp check_completed_tasks(tasks, timeout) do
    ...
    |> case do
      {completed, still_running} ->
        completed_results =
          Enum.map(completed, fn {{_task, {:ok, result}}, {_task_ref, index}} ->
            {index, result}
          end)
  ...
  defp execute_stream_operation(:walk, target, oid, opts),
    do: SnmpKit.SnmpKit.SnmpMgr.walk(target, oid, opts)
