defmodule SnmpKit.SnmpKit.SnmpMgr.Types do
  ...
  def decode_value({:ipAddress, {a, b, c, d}})
  ...
  def infer_type(value) when is_integer(value) and value >= 0 and value < 4_294_967_296,
    do: :unsigned32

  def infer_type(value) when is_integer(value) and value >= 4_294_967_296, do: :counter64
  ...
  defp encode_with_inferred_type(value, :ipAddress)
       when is_tuple(value) and tuple_size(value) == 4 do
    {:ok, {:ipAddress, value}}
  end
  ...
  defp encode_with_explicit_type(value, :ipAddress)
       when is_tuple(value) and tuple_size(value) == 4 do
    {:ok, {:ipAddress, value}}
  end
  ...
  defp encode_with_explicit_type(value, :objectIdentifier) when is_list(value) do
    if Enum.all?(value, &is_integer/1) do
      ...
      {:error,
       "OID values must be numeric integers, found non-integer elements in #{inspect(value)}"}
    end
  end
