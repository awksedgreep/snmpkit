defmodule SnmpKit.SnmpMgr.Table do
  ...
  def to_table(oid_type_value_tuples, table_oid) when is_binary(table_oid) do
    case SnmpKit.SnmpLib.OID.string_to_list(table_oid) do
  ...
  def filter_by_index(table_data, index_filter)
      when is_map(table_data) and is_function(index_filter, 1) do
    filtered =
      table_data
  ...
  def filter_by_column(table_data, column, filter_fn)
      when is_map(table_data) and is_function(filter_fn, 1) do
    filtered =
      table_data
  ...
  def group_by_column(table_data, column) when is_map(table_data) do
    grouped =
      ...
      |> Enum.map(fn {group_value, entries} ->
        rows =
          Enum.map(entries, fn {index, row_data} ->
  ...
  def validate(table_data, opts \\ []) when is_map(table_data) do
    ...
    issues =
      if length(all_column_sets) > 1 do
    ...
    issues =
      if Keyword.get(opts, :check_required_columns) do
  ...
  defp analyze_column_types(table_data, columns) do
    ...
    |> Enum.map(fn column ->
      sample_values =
        ...
        |> Enum.map(&Map.get(&1, column))
        |> Enum.filter(&(&1 != nil))
        # Sample first 10 non-nil values
        |> Enum.take(10)
  ...
  defp detect_numeric_columns(table_data) do
    ...
    |> Enum.filter(fn {_column, values} ->
      ...
      # More than half are numeric
      numeric_count > length(sample_values) / 2
    end)
