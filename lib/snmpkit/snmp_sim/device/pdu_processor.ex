defmodule SnmpKit.SnmpSim.Device.PduProcessor do
  @moduledoc """
  Handles SNMP PDU processing for devices.
  """
  require Logger
  alias SnmpKit.SnmpSim.Device.WalkPduProcessor
    ...
    Enum.map(normalized_bindings, fn oid ->
      ...
      case result do
        {:ok, {_oid_str, type, value}} ->
          Logger.debug(
            "DEBUG: get_dynamic_oid_value returned type=#{inspect(type)}, value=#{inspect(value)}"
          )
          ...
          {:ok, next_oid} ->
            ...
            case get_dynamic_oid_value(next_oid, state) do
              ...
              {:error, _} ->
                ...
                SnmpKit.SnmpSim.Device.OidHandler.get_fallback_next_oid(oid, state)
            end
  ...
  defp get_bulk_repetitions(start_oid, state, max_repetitions) do
    ...
    {_final_oid, results} =
      Enum.reduce_while(1..max_repetitions, {start_oid, []}, fn _, {current_oid, acc} ->
