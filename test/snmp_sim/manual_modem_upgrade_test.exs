defmodule SnmpKit.SnmpSim.ManualModemUpgradeTest do
  ...
  describe "manual modem upgrade writable OIDs" do
    test "server accepts valid IpAddress strings and rejects invalids", %{
      target: target,
      modem: modem
    } do
      # Good IPv4
      ...
      case SnmpKit.SnmpMgr.get(target, @server_oid,
             community: modem.community,
             version: :v2c,
             timeout: 300
           ) do
      ...
      for bad <- ["999.1.1.1", "1.2.3", "abc", "1.2.3.4.5"] do
        case SnmpKit.SnmpMgr.set(target, @server_oid, bad,
               community: modem.community,
               version: :v2c,
               timeout: 300
             ) do
    ...
    test "filename enforces length and type", %{target: target, modem: modem} do
      ...
      case SnmpKit.SnmpMgr.set(target, @filename_oid, long_name,
             community: modem.community,
             version: :v2c,
             timeout: 300
           ) do
      ...
      case SnmpKit.SnmpMgr.set(target, @filename_oid, {:integer, 123},
             community: modem.community,
             version: :v2c,
             timeout: 300
           ) do
    ...
    test "admin status triggers upgrade only with valid server and filename", %{
      target: target,
      modem: modem
    } do
      ...
      case SnmpKit.SnmpMgr.get(target, @oper_oid,
             community: modem.community,
             version: :v2c,
             timeout: 300
           ) do
  ...
  describe "error cases and notWritable enforcement" do
    test "admin status rejects trigger when preconditions invalid", %{
      target: target,
      modem: modem
    } do
      # Without valid server/filename
      case SnmpKit.SnmpMgr.set(target, @admin_oid, {:integer, 1},
             community: modem.community,
             version: :v2c,
             timeout: 300
           ) do
    ...
    test "non-upgrade OIDs reject SET with notWritable", %{target: target, modem: modem} do
      ...
      case SnmpKit.SnmpMgr.set(target, "1.3.6.1.2.1.1.1.0", "x",
             community: modem.community,
             version: :v2c,
             timeout: 300
           ) do
