defmodule SnmpKit.SnmpLib.DashboardTest do
  ...
  describe "Dashboard.record_metric/3" do
    ...
    test "records SNMP operation metrics" do
      :ok = Dashboard.record_metric(:snmp_response_time, 125, %{
        device: "192.168.1.1",
        operation: "get",
        status: :success
      })
      
      :ok = Dashboard.record_metric(:snmp_errors, 1, %{
        device: "192.168.1.1",
        error_type: "timeout"
      })
    end
  ...
  describe "Dashboard.create_alert/3" do
    ...
    test "creates device-specific alerts" do
      :ok = Dashboard.create_alert(:device_unreachable, :critical, %{
        device: "192.168.1.1",
        last_seen: DateTime.utc_now(),
        consecutive_failures: 5
      })
      
      :ok = Dashboard.create_alert(:slow_response, :warning, %{
        device: "192.168.1.1",
  ...
  describe "Dashboard.get_timeseries/3" do
    ...
    test "returns timeseries data for metrics" do
      ...
      device_timeseries = Dashboard.get_timeseries(:snmp_response_time, 3_600_000, %{device: "device1"})
