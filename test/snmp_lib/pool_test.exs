defmodule SnmpKit.SnmpLib.PoolTest do
  ...
  setup do
    ...
    pool_names = [:test_pool, :custom_pool, :duplicate_test, :affinity_pool, 
                  :fifo_pool, :rr_pool, :overflow_pool, :max_overflow_pool,
                  :health_pool, :cleanup_pool, :stats_pool, :operation_stats_pool,
                  :error_pool, :recovery_pool, :lifecycle_pool, :timeout_pool,
                  :perf_pool, :reuse_pool]
  ...
  describe "Pool.start_pool/2" do
    test "starts a basic pool with default options" do
      ...
      assert stats.size == 10  # Default size
  ...
  describe "Pool.with_connection/4" do
    ...
    test "executes function with pooled connection" do
      result = Pool.with_connection(:test_pool, "192.168.1.1", fn conn ->
        assert is_map(conn)
    ...
    test "handles function errors gracefully" do
      result = Pool.with_connection(:test_pool, "192.168.1.1", fn _conn ->
        raise "Test error"
      end)
    ...
    test "supports concurrent operations" do
      ...
      assert stats.active_connections == 0  # All returned
    end
  end
  ...
  describe "Pool.checkout_connection/3 and checkin_connection/2" do
    ...
    test "handles checkout timeout when pool is exhausted" do
      ...
      assert {:error, :no_connections} = Pool.checkout_connection(:test_pool, "device3", timeout: 100)
  ...
  describe "Pool overflow handling" do
    ...
    test "rejects connections when max overflow is reached" do
      ...
      assert {:error, :no_connections} = Pool.checkout_connection(:max_overflow_pool, "device3", timeout: 100)
  ...
  describe "Pool error handling" do
    test "handles worker process failures gracefully" do
      ...
      result = Pool.with_connection(:error_pool, "device1", fn conn ->
        assert is_map(conn)
        :ok
      end)
      ...
      {time_microseconds, _} = :timer.tc(fn ->
        Enum.each(1..20, fn _i ->
          Pool.with_connection(:reuse_pool, device, fn _conn ->
            :timer.sleep(5)  # Simulate SNMP operation
            :ok
          end)
        end)
      end)
