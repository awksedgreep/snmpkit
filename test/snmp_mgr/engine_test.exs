defmodule SnmpKit.SnmpMgr.EngineComprehensiveTest do
      ...
      case SnmpMgr.engine_request(request) do
        {:ok, result} ->
          case result do
            ...
            %{error: error_reason} ->
              assert is_atom(error_reason), "Engine error should be descriptive: #{inspect(error_reason)}"
      ...
      case SnmpMgr.engine_batch(batch_requests) do
        {:ok, results} ->
          ...
          for {result, i} <- Enum.with_index(results) do
            case result do
              {:ok, response} ->
                assert is_map(response) or is_binary(response), "Batch result #{i} should be valid"
    ...
    test "validates batch request optimization" do
      ...
      case SnmpMgr.engine_batch(optimized_batch) do
        {:ok, results} ->
          ...
          assert avg_time_per_request < 100_000, # Less than 100ms per request on average
  ...
  describe "engine performance and concurrency" do
    ...
    test "validates engine memory usage under load" do
      ...
      assert memory_per_request < 10_000, # Less than 10KB per request
    ...
    test "validates engine queue management" do
      ...
      queue_test_requests = for i <- 1..20 do
        %{
          ...
          community: "public",
          request_id: "queue_#{i}",
          timeout: 100  # Short timeout to test queuing
        }
      end
  ...
  describe "engine error handling and resilience" do
    test "validates engine handling of network failures" do
      unreachable_request = %{
        type: :get,
        target: "192.0.2.1", # RFC 5737 test network
        oid: @test_oids.system_descr,
        community: "public",
        timeout: 1000,
        retries: 0
      ...
      for {malformed_request, i} <- Enum.with_index(malformed_requests) do
        case SnmpMgr.engine_request(malformed_request) do
          {:ok, result} ->
            case result do
              %{error: error_reason} ->
                assert is_atom(error_reason), "Engine should reject malformed request #{i}: #{error_reason}"
        ...
        {:ok, result} ->
          ...
          case result do
            %{error: :timeout} ->
              ...
              min_expected_time = 3 # 3 attempts * 1ms timeout
    ...
    test "validates engine resource cleanup" do
      ...
      cleanup_requests = for i <- 1..20 do
        %{
          type: :get,
          target: "192.0.2.#{rem(i, 5) + 1}", # Unreachable targets
          oid: @test_oids.system_descr,
          community: "public",
          timeout: 100,
          retries: 0
      ...
      case results do
        {:ok, batch_results} ->
          ...
          assert length(batch_results) == 20, "Engine should handle failing requests without leaks"
          
          # Most should be errors
          error_count = Enum.count(batch_results, fn
            {:ok, %{error: _}} -> true
            {:error, _} -> true
            _ -> false
          end)
    ...
    test "validates engine circuit breaker protection" do
      ...
      case results do
        {:ok, batch_results} ->
          ...
          circuit_breaker_errors = Enum.count(batch_results, fn
          ...
          if circuit_breaker_errors > 0 do
            ...
            assert true, "Engine processed failing requests (circuit breaker may not be triggered)"
          end
