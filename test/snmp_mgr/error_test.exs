defmodule SnmpKit.SnmpMgr.ErrorComprehensiveTest do
  ...
  describe "Error Format Consistency with SnmpLib" do
    ...
    test "all SNMP operations return consistent error formats", %{device: device} do
      ...
      operations = [
        ...
        {:walk, fn -> SnmpKit.SnmpMgr.walk(target, "invalid.oid", timeout: 100) end}
      ]
  ...
  describe "SnmpKit.SnmpLib.OID Error Integration" do
    setup do
      {:ok, device} = SNMPSimulator.create_test_device()
      :ok = SNMPSimulator.wait_for_device_ready(device)
    ...
    test "invalid OID formats handled by SnmpKit.SnmpLib.OID", %{device: device} do
      target = SNMPSimulator.device_target(device)

      # Test invalid OID handling through SnmpKit.SnmpLib.OID
      invalid_oids = [
        "invalid.oid.format",
        "",
        "not.numeric.oid",
      ...
      Enum.each(invalid_oids, fn oid ->
        ...
        case result do
          {:error, reason} ->
            # Should get proper error from SnmpKit.SnmpLib.OID validation
            assert is_atom(reason) or is_tuple(reason),
    ...
    test "OID processing errors consistent across operations", %{device: device} do
      ...
      operations = [
        ...
        fn ->
          SnmpMgr.set(target, invalid_oid, "value", community: device.community, timeout: 100)
        end,
        fn ->
          SnmpMgr.get_bulk(target, invalid_oid,
            max_repetitions: 3,
        ...
        fn ->
          SnmpKit.SnmpMgr.walk(target, invalid_oid, community: device.community, timeout: 100)
        end
      ]
  ...
  describe "Error Handling Performance with SnmpLib" do
    ...
    test "error handling doesn't impact performance", %{device: device} do
      ...
      error_operations =
        Enum.map(1..10, fn i ->
          Task.async(fn ->
            SnmpMgr.get(target, "invalid.oid.#{i}", community: device.community, timeout: 100)
          end)
        end)
      ...
      # Less than 2 seconds for 10 error operations
      ...
      concurrent_errors =
        Enum.map(1..5, fn i ->
          Task.async(fn ->
            case rem(i, 3) do
              # Network error
              0 -> SnmpMgr.get("240.0.0.1", "1.3.6.1.2.1.1.1.0", timeout: 50)
              # OID error
              1 -> SnmpMgr.get(target, "invalid.oid.#{i}", timeout: 100)
              # Auth error
              2 -> SnmpMgr.get(target, "1.3.6.1.2.1.1.1.0", community: "wrong", timeout: 100)
            end
          end)
        end)
  ...
  describe "SNMPv2c Exception Values through SnmpLib" do
    ...
    test "SNMPv2c exception values handled by snmp_lib", %{device: device} do
      ...
      Enum.each(exception_test_cases, fn {oid, expected_exceptions} ->
        result =
          SnmpMgr.get(target, oid, version: :v2c, community: device.community, timeout: 100)
    ...
    test "bulk operations handle exceptions through snmp_lib", %{device: device} do
      ...
      result =
        SnmpMgr.get_bulk(target, "1.3.6.1.2.1.999",
          max_repetitions: 5,
          version: :v2c,
          community: device.community,
          timeout: 100
  ...
  describe "Application-Level Error Enhancement" do
    ...
    test "consistent error handling across all API functions", %{device: device} do
      ...
      api_functions = [
        ...
        {:get_bulk,
         fn ->
           SnmpMgr.get_bulk("240.0.0.1", "1.3.6.1.2.1.2.2", max_repetitions: 3, timeout: 50)
         end},
