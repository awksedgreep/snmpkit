...
Run all walk tests:
```bash
mix test test/snmp_mgr/walk_*_test.exs
```

Run specific test file:
```bash
mix test test/snmp_mgr/walk_unit_test.exs
mix test test/snmp_mgr/walk_comprehensive_test.exs
mix test test/snmp_mgr/walk_integration_test.exs
mix test test/snmp_mgr/walk_regression_test.exs
```
...
mix test test/snmp_mgr/walk_regression_test.exs -t regression_critical

# Type preservation (prevents type information loss)
mix test test/snmp_mgr/walk_comprehensive_test.exs --only type_preservation
```
...
```bash
# Increase timeout
mix test test/snmp_mgr/walk_*_test.exs --timeout 60000
```
...
mix test test/snmp_mgr/walk_regression_test.exs --trace

# Run single test
mix test test/snmp_mgr/walk_regression_test.exs:67
```
...
```yaml
- name: Run Walk Tests
  run: mix test test/snmp_mgr/walk_*_test.exs --timeout 30000
```
...
4. Ensure all tests pass: `mix test test/snmp_mgr/walk_*_test.exs`
